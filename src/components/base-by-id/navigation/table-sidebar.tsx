...
export function TableSidebar({ sidebarOpen }: Props) {
  return (
    <div
      className={`relative h-full border-r border-gray-200 bg-white transition-[width] duration-600 ease-out ${sidebarOpen ? "w-[320px]" : "w-0"} `}
    >
      ...
      >
        <div className="space-y-1 px-2 py-2">
          ...
          <button className="flex h-9 w-full items-center gap-2 rounded px-2 text-gray-500 hover:bg-gray-100">
            <Search size={20} />
            <span className="text-[12px] text-gray-500">Find a view</span>
          </button>

          <button className="pointer flex h-9 w-full items-center gap-2 rounded px-2 hover:bg-gray-100">
            <TableCellsSplit size={20} className="text-blue-600" />
            <span className="text-[12px]">Grid View</span>
          </button>
        </div>
      </div>
    </div>
